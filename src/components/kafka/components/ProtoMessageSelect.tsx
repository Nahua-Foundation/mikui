...
export function ProtoMessageSelect({
  ...
  return (
    // `modal` — из-за колеса мыши, а не из-за поведения поверх остального.
    //
    // Оба места, где стоит этот выбор, — внутри модального окна, а окно держит
    // `react-remove-scroll`: тот гасит `wheel` всюду, кроме собственного узла с
    // содержимым окна. Попоувер живёт в портале у `body`, то есть снаружи, и
    // колесо над списком до него просто не доходило — стрелки и поиск работали,
    // прокрутка нет. Со `Select`, который стоял здесь раньше, этого не было
    // видно: он модальный по умолчанию и ставит собственный замок.
    //
    // `modal` как раз и заводит такой замок вокруг содержимого попоувера —
    // верхний в стопке, поэтому замок окна отступает, и колесо снова доходит.
