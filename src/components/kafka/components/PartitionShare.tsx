...
/**
 * Категориальная палитра, одна на обе темы, в ФИКСИРОВАННОМ порядке.
 *
 ...
 * приложения (`--color-surface`) в обеих темах — #0f172b и #ffffff: худшая
 * соседняя пара по CVD ΔE 8.4, по обычному зрению 19.3, контраст всех восьми к
 * фону выше 3:1 и там, и там. Менять состав или порядок — только прогнав
 * валидатор заново, причём по обеим поверхностям.
 *
 * Своей редакции для светлой темы у палитры поэтому нет, и это не экономия:
 * восемь тонов уже стоят в той полосе светлоты, которая держит контраст с
 * обоих концов шкалы, — вторая редакция только увела бы соседние пары друг к
 * другу. Единственное, что светлая тема меняет в полосе, — цвет просветов
 * между заливками: они показывают `surface` под собой.
 *
