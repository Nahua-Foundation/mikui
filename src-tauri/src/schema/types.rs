...
pub struct SchemaFile {
    ...
    pub source: String,
    /// Файл не выбирали — его нашли по `import` (см. `proto::resolve`).
    ///
    /// Различать обязательно, и по трём причинам. Разбору: типы зависимостей
    /// не должны попадать в список message на выбор — декодировать топик
    /// `common_proto.Metadata` никто не собирается, это такой же мусор в
    /// списке, как `google.protobuf.Timestamp`. Операциям: удалять и
    /// перечитывать по одному можно то, что добавляли, а зависимости
    /// пересчитываются целиком при каждом изменении набора. И форме: показать
    /// сорок подтянутых файлов вперемешку с двумя выбранными значит спрятать
    /// выбранные.
    ///
    /// `#[serde(default)]` — ключа нет у всего, что сохранено прошлыми
    /// версиями, и там все файлы выбраны руками: `false` и есть верное
    /// прочтение такой записи.
