...
/// Сколько всего ждать ответа кластера при подключении.
///
/// Было 5 секунд «чтобы человек не смотрел на зависшее приложение», и ровно
/// этот потолок не давал подключиться к общему кластеру: цепочка TLS →
/// ApiVersions → SASL занимает доли секунды только на ненагруженном стенде, а
/// на кластере с квотой брокер вправе придержать ответ на всё квотное окно
/// (`quota.window.num × quota.window.size.seconds`, по умолчанию около 11
/// секунд). Пять секунд — короче этого окна, то есть подключение обрывалось
/// нами самими; ровно та же ошибка когда-то стоила нам чтения, когда
/// `socket.timeout.ms` был выставлен в 10 секунд (см. `helpers::base_config`).
///
/// Отзывчивость от этого не страдает, потому что до потолка доживает только
/// молчащий кластер. Всё, что можно назвать ошибкой НАСТРОЙКИ — неверный
/// пароль, не тот CA, неподдерживаемый механизм, — librdkafka распознаёт за
/// доли секунды и кричит об этом колбэком, а `is_fatal` обрывает ожидание
/// сразу же.
const CONNECT_TIMEOUT: Duration = Duration::from_secs(25);

/// Сколько ждать кластер, который не ответил НИ РАЗУ.
///
/// Полный бюджет отмерен молчанию брокера, с которым уже установлена связь, —
/// платить им же за опечатку в адресе не надо: неверный хост или закрытый порт
/// это `BrokerTransportFailure` на каждой попытке подряд, и двадцать пять
/// секунд такого — это только двадцать пять секунд смотрения на спиннер.
///
/// Десять секунд, а не прежние пять, потому что в этот же промежуток попадает
/// медленный DNS и вся цепочка TLS → ApiVersions → SASL: пока брокер не дошёл
/// до UP, librdkafka отвечает на запрос метаданных тем же самым
/// `BrokerTransportFailure`, что и при полном отсутствии хоста, и отличить
/// «не достучались» от «ещё здороваемся» по одной попытке нельзя.
const CONNECT_UNREACHABLE_TIMEOUT: Duration = Duration::from_secs(10);

/// Первый шаг проверки связи. Он же — задержка, с которой замечается ошибка
/// аутентификации: она приезжает колбэком между попытками.
const CONNECT_PROBE_STEP: Duration = Duration::from_millis(300);

/// Потолок шага: на каждой неудаче он удваивается до этой величины.
///
/// Фиксированные 300 мс были второй половиной той же беды. Каждый шаг — это
/// отдельный `MetadataRequest` с флагом `force`, и ответ, не успевший за шаг,
/// не ждётся и не кэшируется, а выбрасывается вместе с очередью ответа
/// (`rd_kafka_metadata()`). На кластере, который отвечает дольше 300 мс,
/// проверка не могла закончиться успехом НИКОГДА, сколько ни дай общего
/// времени: она лишь выстреливала полтора десятка запросов подряд, добавляя
/// брокеру поводов придержать следующий ответ по квоте.
///
/// Рост вдвое оставляет быстрый кластер быстрым (он отвечает на первом же
/// шаге), а медленному даёт дожить до ответа за единицы попыток.
const CONNECT_PROBE_STEP_MAX: Duration = Duration::from_secs(2);

/// Имя топика, метаданные которого спрашиваются при проверке связи.
///
/// Спрашивать нужно именно один топик, а не весь кластер. `fetch_metadata`
/// без имени — это `all_topics=1`, то есть просьба перечислить ВСЁ: на общем
/// кластере с тысячами топиков ответ измеряется мегабайтами и секундами, и
/// платить этим за проверку «жив ли брокер» незачем. Ответ про один топик
/// приходит крошечным, но приезжает по тому же соединению и тем же способом —
/// а список брокеров и `cluster.id` в нём есть в любом случае.
///
/// Топика с таким именем в кластере заведомо нет, и это не мешает:
/// несуществующий топик — это код ошибки ВНУТРИ ответа, а не отказ на сам
/// запрос. Создать его побочным эффектом тоже нельзя, `allow.auto.create.topics`
/// выключен (см. `helpers::consumer_config`).
const CONNECT_PROBE_TOPIC: &str = "__mikui_connect_probe";

/// Сколько ждать `cluster.id`. Короткий и без последствий: ответ с
/// метаданными к этому моменту уже пришёл (`wait_until_ready`) и лежит в кэше
/// librdkafka — `cluster.id` едет в любом Metadata-ответе, в том числе в
/// ответе про один топик. Неудача стоит ровно одного — ссылкой на сообщение
/// из этого сеанса не поделиться. Задерживать ради этого подключение нельзя.
...
///
/// Успех этой проверки означает больше, чем кажется: ответ на метаданные
/// librdkafka спрашивает только у брокера в состоянии UP
/// (`rd_kafka_broker_any_usable`), а в UP брокер переходит последним шагом
/// цепочки CONNECT → SSL_HANDSHAKE → APIVERSION_QUERY → AUTH_HANDSHAKE →
/// AUTH_REQ. То есть дошли сюда — значит и TLS, и SASL уже приняты кластером.
/// Обратное тоже полезно при разборе жалоб: `Timed out` здесь — это молчание
/// в ответ на УЖЕ отправленный запрос метаданных, а «не удалось подключиться и
/// аутентифицироваться» librdkafka называет иначе (`_TRANSPORT`).
fn wait_until_ready(
    consumer: &BaseConsumer<MeteredContext>,
    fatal: &FatalError,
) -> Result<(), String> {
    let started = Instant::now();
    let mut step = CONNECT_PROBE_STEP;
    // Добрался ли хоть один запрос до брокера. Пока нет — ждём по короткому
    // бюджету, см. `CONNECT_UNREACHABLE_TIMEOUT`.
    ...
    loop {
        ...
        let failure = match consumer.fetch_metadata(Some(CONNECT_PROBE_TOPIC), step) {
            Ok(_) => return Ok(()),
            Err(e) => e,
        };
        reached |= !is_unreachable(&failure);

        let budget = if reached {
            CONNECT_TIMEOUT
        } else {
            CONNECT_UNREACHABLE_TIMEOUT
        };
        if started.elapsed() >= budget {
            ...
            return Err(match fatal.take() {
                Some(reason) => reason,
                None => format!("can't reach cluster: {failure}"),
            });
        }

        // Кластер не уложился в шаг — даём следующему больше времени. Ответ на
        // нынешний запрос librdkafka уже выбросила, ждать его некому.
        step = (step * 2).min(CONNECT_PROBE_STEP_MAX);
    }
}

/// Означает ли ошибка запроса метаданных, что до брокера так и не достучались.
///
/// Разница видна прямо в `rd_kafka_metadata()`: запрос отправляется только
/// брокеру в состоянии UP, и если такого не нашлось за отведённое время, она
/// возвращает `_TRANSPORT`, даже не сходив в сеть. `_TIMED_OUT` — это уже
/// про отправленный запрос, на который не пришёл ответ, то есть про кластер,
/// который на связи и просто медлит.
fn is_unreachable(error: &KafkaError) -> bool {
    matches!(
        error,
        KafkaError::MetadataFetch(RDKafkaErrorCode::BrokerTransportFailure)
    )
}
