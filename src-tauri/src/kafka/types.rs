...
pub struct MessageFilter {
    /// Запрос из поля поиска в шапке: совпадение ГДЕ УГОДНО — в ключе, в имени
    /// или значении любого заголовка, в теле.
    ///
    /// Отдельно от `value`, а не вместо него, и это осознанно. Поле в шапке
    /// одно и стоит на виду, поэтому читается как «найди мне это в
    /// сообщениях», а не «в теле сообщения» — и раньше врало: искало только по
    /// телу. Прицельные поля никуда не делись, они в панели фильтров; здесь —
    /// тот запрос, для которого пользователь ещё не решил, где именно искать.
    #[serde(default)]
    pub anywhere: String,
    #[serde(default)]
    pub key: String,
    /// Запрос по заголовкам. Совпадением считается и имя, и значение: в
    /// заголовках лежат и `traceparent`, и `__TypeId__`, и искать по ним
    /// приходится с обеих сторон.
    #[serde(default)]
    pub headers: String,
    #[serde(default)]
    pub value: String,
...
pub struct RowPreview {
    ...
    ///
    /// Отдельным полем, а не приписью к `preview`, ради таблицы: она рисует
    /// метку тем же приёмом, что `[binary]` и `[undecoded]`, — то есть внутри
    /// колонки тела, не трогая ни состав колонок, ни их растягивание.
...
///
/// Набор вариантов совпал с `schema::BodyFormat`, но типы остаются разными
/// намеренно: этот отвечает на вопрос «как прочитать введённое», тот — «как
/// показать прочитанное». Совпадение случайное и временное — стоит завестись
/// формату, который можно показать, но нельзя набрать руками (или наоборот),
/// как общий тип пришлось бы разбирать обратно на два.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Deserialize, Serialize)]
...
pub struct ProduceRequest {
    ...
    /// Subject реестра, которым кодировать тело. То же самое, что `message`, но
    /// для `avro`: пустой — взять то, что назначено топику.
    #[serde(default)]
